...
13. Optionally add `CODEX_CI_WORKFLOW_FILE` (default `ci.yml`), `CODEX_CI_DISCOVERY_SECONDS` (default `60`), and `CODEX_CI_DISCOVERY_POLL_SECONDS` (default `10`) for the direct-event fallback probe.
14. Set `ARBITER_AUTOPOST=1` only when the repository should allow the model-free arbiter to create or edit its per-PR disposition comment. Unset means summary-only evaluation. `ARBITER_SOFT_GATE` (default `5`), `ARBITER_HARD_CAP` (default `10`), `ARBITER_STUCK_P1_ROUNDS` (default `3`), and `ARBITER_UNVERIFIABLE_ROUNDS` (default `2`) are strict positive-integer termination controls.
15. Create the labels `codex-review` and `codex-triage` if scheduled review is wanted.

`CODEX_MODEL`, `CODEX_REASONING_EFFORT`, `CODEX_MAX_INPUT_BYTES`, `CODEX_MAX_OUTPUT_TOKENS`, `CODEX_MAX_OUTPUT_BYTES`, `CODEX_REQUEST_TIMEOUT`, `CODEX_JOB_TIMEOUT_SECONDS`, and `CODEX_BOT_LOGIN` are configuration, not secrets. Use `none` when selecting a model that does not support reasoning effort; this omits the reasoning parameter from the API request. Higher effort improves difficult payment/security reviews but increases latency and cost; `low` is better for high-volume scheduled triage.
...
`.github/workflows/codex-pr-review.yml` runs normal push reviews from `workflow_run: completed` for `CI`. It checks the completed run's `head_sha`, resolves the open PR, and reviews only that exact head. `reopened`, `ready_for_review`, manual, and scheduled paths remain available without polling. `opened` and `synchronize` remain as a coverage fallback for conflicting heads whose `CI` workflow is never created; they probe for the exact-head CI run and defer with exit zero whenever one exists or appears during the discovery window. A probe error is treated as no run and reviews proceed, so a failed lookup cannot silently delete coverage.

All review paths use a PR-keyed concurrency group, including workflow runs whose payload has no pull-request number. The trusted job checks out only the default branch, executes no PR code, downloads no CI artifacts or caches, and reads exact-head check runs only from `repos/{repo}/commits/{head_sha}/check-runs`. Completed checks are sanitized and passed as bounded untrusted evidence; if no completed check is visible, the input says verification was not available at review time. The named files in `.github/codex/context-files.txt` are a separate, bounded reference set read from `TRUSTED_SHA`; they are reference material, not policy.
