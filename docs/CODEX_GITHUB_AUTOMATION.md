...
9. Optionally add `CODEX_REQUEST_TIMEOUT`. It defaults to `900` seconds. It must cover the generation `CODEX_MAX_OUTPUT_TOKENS` allows — roughly 20 seconds per 1000 tokens — and the script refuses a pair that cannot, so raising the token cap forces a decision about the timeout. A timeout shorter than the generation aborts the request mid-call and posts no review at all.
   ...
   It is bounded above as well as below. `CODEX_JOB_TIMEOUT_SECONDS` (default `1200`, and required to equal the workflow's `timeout-minutes` — the automation test fails if the two drift) is the job's wall clock, and the worker refuses a request timeout that does not leave `180` seconds inside it to sanitize and post the comment. Without that ceiling a configured `CODEX_REQUEST_TIMEOUT=1800` is accepted and then killed by GitHub at 1200 seconds, which is the same "no review posted" outcome reached from the other side.
