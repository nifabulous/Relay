...
- The case panel shows the case title, one-sentence purpose, current step, estimated duration, state, progress, and one action. It does not show a countdown because case sessions do not track elapsed time.
...
At 768px and below, stack the case first and practice second. The two-column treatment is reserved for 1024px and wider.
...
- The dominant case is selected deterministically: the most recently updated in-progress case wins; if none exists, the first not-started case wins; if all cases are complete, the most recently updated completed case is shown for review. Catalog order is the tie-breaker when timestamps are absent or equal.
- Other cases remain available below the dominant panel as compact secondary entries. They must not compete visually with the selected case.
...
- `CaseSession.updatedAt` for deterministic dominant-case selection, with catalog order as the stable tie-breaker.
...
The user has a payment question and wants to find the right bank by name or BIC, payment scheme, glossary term, lesson, or related destination without guessing which page owns the answer.
...
- The header uses `Explore` and `Search banks by name or BIC, corridors, payment schemes, glossary terms, lessons, and tools.`
...
- Groups are ordered by user value: Banks, Payment schemes, Glossary, Lessons, Tools. Empty groups are omitted rather than rendered as dead space.
...
- A query with no matches explains what types are searched and offers a concrete recovery: try a bank name, an 8- or 11-character BIC, scheme, lesson, or glossary term.
- Bank search is backed by the existing curated directory. Name queries return institution matches with country, city, and BIC context; an uppercase 8- or 11-character BIC query shows a `Look up bank by BIC` action that opens Bank Directory directly.
- Directory-backed bank results have their own loading/error state and must not hide or invalidate glossary, lesson, scheme, or tool results.
- Result type is always visible. A glossary definition must not look like a bank record, and a lesson must not look like an executable tool. Tools remain a distinct fifth group after Lessons.
- Selecting a result navigates directly to the destination and preserves the query context when the user returns.
- Search input keeps focus on open, arrow keys move the active result, Enter opens it, and Escape closes the overlay when search is presented as a shell-owned command surface. A deep link such as `/explore?q=IBAN` opens its result list immediately without stealing focus.
- Recent searches are helpful memory, not a source of truth. Store at most five normalized queries in local-only versioned storage, newest first, deduplicated, with individual removal. Corrupt history resets to an empty list without blocking search.
...
- The existing directory-backed bank-name search and BIC handoff to Bank Directory.
...
- The workspace label uses `Operate`; the page `h1` is `Prepare a payment` with the purpose statement `Prepare, validate, and understand a simulated payment.`
...
- Keep the step indicator readable. Use the full horizontal treatment at 1024px and wider; use a compact vertical treatment below 1024px rather than choosing between variants at implementation time.
- Make the primary action full width.
- Keep the primary action reachable after the last required field. At 390px, use a sticky bottom action bar when the form action is outside the viewport; include safe-area padding, reserve layout space, and scroll the first invalid field into view so the bar never covers validation errors.
...
- `Run checks` remains enabled. On submit, it validates the form, moves focus to the first invalid field, and exposes a concise validation summary. This keeps the action discoverable for keyboard and touch users.
...
- After submission, the page shows a real composite-request state: `idle`, `validating`, `checking`, `success`, `partial`, `error`, or `stale`. Because the existing API is one composite request, loading copy must say `Running payment checks…` rather than pretending to stream per-check progress.
- A successful response is `partial` only when one or more sub-results are explicitly unavailable or not checked. Those checks are labelled `Not checked` or `Not available`, never `Failed`. A request-level failure shows one scoped error and retries the full composite request; independent check retries are out of scope until the API supports them.
...
| Explore | Search opens immediately; async bank handoff shows scoped loading only. | Destinations and search guidance explain BIC lookup and searchable static content. | No-results recovery or bank-lookup retry preserves query and usable static groups. | Typed grouped results with direct destinations and preserved context. |
| Operate | Form renders immediately; bank context and composite checks load in stable regions. | Required fields explain how to begin; route context asks for a BIC. | Inline field errors, scoped SSI retry, request-level check retry, partial unavailable states, and stale-result recovery. | Results show checks, recommendation, route explanation, provenance, and next action. |
...
- Explore: `frontend/src/features/explore/ExplorePage.tsx`, `frontend/src/features/explore/ExplorePage.css`, `frontend/src/features/explore/search/CommandSearch.tsx`, search-index/history tests, Bank Directory handoff tests, and Explore E2E coverage.
...
- Learn selects the dominant case deterministically and keeps secondary cases available without competing with it.
- Explore makes search the dominant action, distinguishes Banks, Payment schemes, Glossary, Lessons, and Tools, and offers directory-backed bank-name search plus a direct BIC handoff.
- Operate makes the form-to-checks-to-route sequence obvious.
- Operate keeps `Run checks` discoverable, reports the real composite-request state, and never presents unavailable sub-results as failed checks.
- All three pages retain their existing routes and core data behavior.
- Every page has intentional first-use, loading, empty, error, and success states; composite or asynchronous surfaces also cover partial results where applicable.
- Mobile layouts are designed at 390px and do not require horizontal scrolling. Learn and Operate stack below 1024px; Operate reserves space for its mobile action bar.
...
1. Add or update focused unit tests for Learn case selection, resume/start/completed/corrupt states, Explore group order/BIC handoff/deep-linked query/recent-history recovery, and Operate validation/composite states/stale results.
2. Add Playwright coverage for Learn resume/start, Explore grouped search and no-results recovery, BIC handoff, and Operate validation/checks/partial/error/stale-result states.
...
The visual directions and UX adjustments are approved in conversation. This revised written spec incorporates the review decisions around directory-backed bank-name and BIC search, deterministic case selection, composite check states, recent history, responsive breakpoints, and mobile action reachability. It must be approved before creating the execution plan. After approval, the next step is an ordered implementation plan with testable tasks.
