...
| Frontend unit/integration tests | **1,237 passing** (`cd frontend && npm test -- --run --no-file-parallelism`; the default parallel run is still load-sensitive for the Case Desk preferred-tier scenario) |
...
| Eager shell bundle | **167,764 bytes gzip** (budget: 204,800 bytes) |
...
| Frontend tests | 1,237 passing (serial-file mode) |
