...
| Frontend unit/integration tests | **1,238 passing** (`cd frontend && npm test -- --run --no-file-parallelism`; the default parallel run is still load-sensitive for the Case Desk preferred-tier scenario) |
...
| Eager shell bundle | **168,037 bytes gzip** (budget: 204,800 bytes) |
...
| Frontend tests | 1,238 passing (serial-file mode) |
