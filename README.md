...
| Frontend unit/integration tests | **1,242 passing** (`cd frontend && npm test -- --run --no-file-parallelism`; the default parallel run is still load-sensitive for the Case Desk preferred-tier scenario) |
...
| Frontend tests | 1,242 passing (serial-file mode) |
...
start. Everything the app reads — 311 banks, 1,572 SSI records, 72 corridor
