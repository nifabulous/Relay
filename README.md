...
| Frontend unit/integration tests | **1,231 passing** (`cd frontend && npm test -- --run --no-file-parallelism`; the default parallel run is still load-sensitive for the Case Desk preferred-tier scenario) |
...
| Eager shell bundle | **167,639 bytes gzip** (budget: 204,800 bytes) |
...
| Frontend tests | 1,231 passing (serial-file mode) |
