"""Focused invariants for India/Sri Lanka SSI wave 10 batch 4."""
...
def test_batch4_has_four_unique_routes():
    rows = _rows()
    assert len(rows) == 4
    keys = [(row[0], row[2], row[3]) for row in rows]
    assert len(keys) == len(set(keys))
    assert {row[0] for row in rows} == {"IDIBLKLCXXX"}


def test_batch4_rows_are_canonical_bic_only_metadata():
    for row in _rows():
        assert len(row) == 15
