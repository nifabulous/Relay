...
def _load():
    ...
    evidence = json.loads((ROOT / "scripts/ssi-autopilot/evidence/ssi-wave28-dnbaus33-2026-02-02.json").read_text())
    bank = next(bank for region in manifest["regions"] for bank in region["banks"] if bank["bic8"] == "DNBAUS33")
    return bank, evidence


def test_wave28_mask_preserves_route_account_identity():
    bank, evidence = _load()
