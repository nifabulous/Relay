...
def test_bivial_live_table_matches_masked_seed_rows():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-wave64-klarch22-2026-09-14.json").read_text()
    )
    beneficiary = evidence["beneficiary"]["bic"]
    rows = [row for row in SSI_RECORDS if row[0] == beneficiary]
    expected = {(route["currency"], route["int_bic"] + "XXX") for route in evidence["routes"]}
...
def test_bivial_rows_are_excluded_by_the_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "KLARCH22XXX"
    ).all()
    assert len(rows) == 17
    assert all(not _is_routable_ssi(row) for row in rows)
    currencies = {row.currency for row in rows}
    assert all(
        suggest_from_ssi(db_session_clean, "KLARCH22XXX", currency, "CH") == []
