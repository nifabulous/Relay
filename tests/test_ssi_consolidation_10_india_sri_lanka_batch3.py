"""Focused invariants for India/Sri Lanka SSI wave 10 batch 3."""
...
def test_batch3_has_seven_unique_routes():
    rows = _rows()
    assert len(rows) == 7
    keys = [(row[0], row[2], row[3]) for row in rows]
    assert len(keys) == len(set(keys))
    assert {row[0] for row in rows} == {"INDBINAAXXX"}


def test_batch3_rows_are_canonical_bic_only_metadata():
    for row in _rows():
        assert len(row) == 15
...
def test_batch3_has_one_official_source():
    sources = {row[9].split(" (as of ", 1)[0] for row in _rows()}
    assert sources == {
        "Source: https://bank.indusind.bank.in/content/dam/indusind-corporate/Other/financial-institutions/ssi-ibu-unit-IBU-effective31-May-2025.pdf"
    }
