...
def test_everbank_foreign_currency_table_matches_masked_seed_rows():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-everbank-2026-09-13.json").read_text()
    )
    ...
    assert len(rows) == evidence["source_snapshot"]["route_count"] == 19
    assert {(row[2], row[3]) for row in rows} == expected
    assert all(row[5].startswith("ACCT-910054") for row in rows)
    assert all(row[6].startswith("ACCT-910054") for row in rows)
...
def test_everbank_rows_are_excluded_by_the_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "EVBKUS3MXXX"
    ).all()
    assert len(rows) == 19
    assert all(not _is_routable_ssi(row) for row in rows)
    currencies = {row.currency for row in rows}
    assert all(
        suggest_from_ssi(db_session_clean, "EVBKUS3MXXX", currency, "US") == []
