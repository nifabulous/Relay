...
def test_jordan_kuwait_bank_table_matches_bic_only_seed_rows():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-wave54-jgbajoa0-2026-09-14.json").read_text()
    )
    beneficiary = evidence["beneficiary_bic"]
    rows = [row for row in SSI_RECORDS if row[0] == beneficiary + "XXX"]
    expected = {
    ...
    assert all("jkb.com" in row[9] for row in rows)
    assert all(not _is_routable_ssi(_row_to_ssi(row)) for row in rows)


def test_jordan_kuwait_bank_rows_are_excluded_by_the_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "JGBAJOA0XXX"
    ).all()
    assert len(rows) == 33
    assert all(not _is_routable_ssi(row) for row in rows)
    currencies = {row.currency for row in rows}
    assert all(
        suggest_from_ssi(db_session_clean, "JGBAJOA0XXX", currency, "JO") == []
