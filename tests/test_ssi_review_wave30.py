...
def test_synovus_bic_only_rows_are_not_routable():
    """Synovus publishes correspondent BICs, not executable settlement terms."""
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-synovus-2026-09-13.json").read_text()
    )
    rows = [row for row in SSI_RECORDS if row[0] == "FICOUS44XXX"]
    actual = [{"currency": row[2], "int_bic": row[3]} for row in rows]
    assert actual == evidence["routes"]
    assert evidence["source_effective_date"] == "2025-09-01"
...
def test_synovus_bic_only_rows_are_excluded_by_live_selection(db_session_clean):
    """The production SSI selector returns no executable Synovus routes."""
    currencies = {row[2] for row in SSI_RECORDS if row[0] == "FICOUS44XXX"}
    assert currencies
    assert all(
        suggest_from_ssi(db_session_clean, "FICOUS44XXX", currency, "US") == []
