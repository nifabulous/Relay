...
def test_hibret_correspondent_table_matches_bic_only_seed_rows():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-wave50-untdetaa-2026-09-13.json").read_text()
    )
    routes = evidence["source_snapshot"]["routes"]
    rows = [row for row in SSI_RECORDS if row[0] == "UNTDETAAXXX"]
...
def test_hibret_bic_only_rows_are_excluded_by_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "UNTDETAAXXX"
    ).all()
    assert len(rows) == 14
