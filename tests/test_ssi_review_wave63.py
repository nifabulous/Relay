...
def test_esaf_routes_match_official_schedule_and_masks():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-wave63-esfainbb-2025-07-01.json").read_text()
    )
    beneficiary = evidence["beneficiary"]["bic"]
    rows = [row for row in SSI_RECORDS if row[0] == beneficiary]
    expected = {
        (route["currency"], route["int_bic"]): route["account_mask"]
        for route in evidence["routes"]
    }
    assert len(rows) == evidence["source_snapshot"]["admitted_route_count"] == 3
    assert {(row[2], row[3]): row[5] for row in rows} == expected
    assert all(row[5] == row[6] for row in rows)
    assert all(row[5].startswith("ACCT-9100209") for row in rows)
...
def test_esaf_routes_are_excluded_by_the_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "ESFAINBBXXX"
    ).all()
    assert len(rows) == 3
    assert all(not _is_routable_ssi(row) for row in rows)
    assert all(
        suggest_from_ssi(db_session_clean, "ESFAINBBXXX", row.currency, "IN") == []
        for row in rows
    )
