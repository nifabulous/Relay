...
def _evidence():
    return json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-wave61-mblbbddh-2026-09-14.json").read_text()
    )


def test_mercantile_live_routes_match_masked_evidence():
    evidence = _evidence()
    ...
    assert len(rows) == evidence["source_snapshot"]["route_count"] == 7
    assert actual == expected
    assert all(re.fullmatch(r"ACCT-910018(?:0[9]|1[0-5])", row[5]) for row in rows)
...
def test_mercantile_inferred_routes_are_excluded_by_production_selector(db_session_clean):
    evidence = _evidence()
    ...
    assert all(
        suggest_from_ssi(db_session_clean, evidence["beneficiary"]["bic"], row.currency, "BD") == []
        for row in rows
    )
