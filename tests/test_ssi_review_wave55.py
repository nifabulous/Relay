...
def test_spuerkeess_table_matches_masked_seed_rows():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-wave55-bceelull-2026-01-12.json").read_text()
    )
    ...
    assert len(rows) == evidence["source_snapshot"]["route_count"] == 21
    assert {(row[2], row[3]) for row in rows} == expected
    assert all(row[5].startswith("ACCT-910056") for row in rows)
    assert all(row[6].startswith("ACCT-910056") for row in rows)
    ...
    assert all("spuerkeess.lu" in row[9] for row in rows)
    assert all(not _is_routable_ssi(_row_to_ssi(row)) for row in rows)
    aud = next(row for row in rows if row[2] == "AUD")
    nzd = next(row for row in rows if row[2] == "NZD")
    assert aud[5] == aud[6] == nzd[5] == nzd[6] == "ACCT-91005601"


def test_spuerkeess_rows_are_excluded_by_the_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "BCEELULLXXX"
    ).all()
    assert len(rows) == 21
    assert all(not _is_routable_ssi(row) for row in rows)
    currencies = {row.currency for row in rows}
    assert all(
        suggest_from_ssi(db_session_clean, "BCEELULLXXX", currency, "LU") == []
