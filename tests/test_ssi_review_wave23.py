...
def _load():
    ...
    evidence = json.loads((ROOT / "scripts/ssi-autopilot/evidence/ssi-wave23-dnbanokk-2026-02-02.json").read_text())
    bank = next(bank for region in manifest["regions"] for bank in region["banks"] if bank["bic8"] == "DNBANOKK")
    return bank, evidence


def test_wave23_mask_preserves_route_account_identity():
    bank, evidence = _load()
    ...
    assert trusted["DNBANOKK"]["settlement_terms_published"] is False
