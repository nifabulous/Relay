...
def test_icici_hk_routes_match_official_correspondent_pdf(db_session_clean):
    """ICICI Hong Kong's official PDF is represented as BIC-only data."""
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-icici-hk-2026-09-13.json").read_text()
    )
    ...
    for row in rows:
        ...
        assert suggest_from_ssi(db_session_clean, evidence["beneficiary_bic"], row[2], "HK") == []
