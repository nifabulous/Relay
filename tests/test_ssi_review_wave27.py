...
def _load():
    ...
    evidence = json.loads((ROOT / "scripts/ssi-autopilot/evidence/ssi-wave27-dnbafihx-2026-02-02.json").read_text())
    bank = next(bank for region in manifest["regions"] for bank in region["banks"] if bank["bic8"] == "DNBAFIHX")
    return bank, evidence


def test_wave27_mask_preserves_route_account_identity():
    bank, evidence = _load()
