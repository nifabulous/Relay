...
def test_jordan_kuwait_bank_bic_only_extract_matches_seed_rows():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-wave68-jkbajoam-2026-09-14.json").read_text()
    )
    ...
    assert len(rows) == evidence["source_snapshot"]["route_count"] == 33
...
def test_jordan_kuwait_bank_rows_are_excluded_by_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "JKBAJOAMXXX"
    ).all()
    assert len(rows) == 33
    assert all(row.bic_only and not _is_routable_ssi(row) for row in rows)
    currencies = {row.currency for row in rows}
    assert currencies == {"AED", "AUD", "CAD", "CHF", "CNY", "EGP", "EUR", "GBP", "JPY", "KWD", "OMR", "QAR", "SAR", "TND", "TRY", "USD"}
    assert all(
        suggest_from_ssi(db_session_clean, "JKBAJOAMXXX", currency, "JO") == []
