...
EASTERN_EUROPE_SSI_COVERAGE = [
    ...
    ('CITICZPXXXX', 'Citibank Europe plc, organizační složka', {'AUD', 'BGN', 'CAD', 'CHF', 'CNY', 'CZK', 'DKK', 'EUR', 'GBP', 'HKD', 'HUF', 'JPY', 'KZT', 'MXN', 'NOK', 'NZD', 'PLN', 'RON', 'SEK', 'TRY', 'USD', 'ZAR'}),
    ('FNNBROBUXXX', 'Nexent Bank N.V. Amsterdam Bucharest Branch', {'AUD', 'CAD', 'CHF', 'DKK', 'EUR', 'GBP', 'HUF', 'JPY', 'NOK', 'PLN', 'RON', 'SEK', 'TRY', 'USD'}),
]
...
HONG_KONG_SSI_COVERAGE = [
    ...
    ('HDFCHKHHXXX', 'HDFC Bank Limited Hong Kong', {'AUD', 'CHF', 'EUR', 'GBP', 'HKD', 'INR', 'JPY', 'USD'}),
]
...
class TestDenmarkSsiCoverage:
    ...
    def test_denmark_banks_have_seeded_ssi_records(self):
        manifest_expected = _manifest_seedable_coverage("denmark")
        generated_expected = {bic: (bank_name, currencies) for bic, bank_name, currencies in DENMARK_SSI_COVERAGE}
        assert generated_expected == manifest_expected
        seeded = {}
        for record in SSI_RECORDS:
            seeded.setdefault(record[0], set()).add(record[2])
    ...
    def test_denmark_banks_are_in_the_bank_directory(self):
        bank_bics = {row[0] for row in BANKS}
        manifest_expected = _manifest_seedable_coverage("denmark")
        missing = [bic for bic in manifest_expected if bic not in bank_bics]
        assert not missing, (
            f"denmark SSI beneficiaries must also be seeded in BANKS so "
            f"Explore can show their settlement instructions: {missing}"
        )
...
class TestSouthernAfricaSsiCoverage:

    def test_southern_africa_banks_have_seeded_ssi_records(self):
        manifest_expected = _manifest_seedable_coverage("southern-africa")
        generated_expected = {bic: (bank_name, currencies) for bic, bank_name, currencies in SOUTHERN_AFRICA_SSI_COVERAGE}
        assert generated_expected == manifest_expected
        seeded = {}
        for record in SSI_RECORDS:
            seeded.setdefault(record[0], set()).add(record[2])
    ...
    def test_southern_africa_banks_are_in_the_bank_directory(self):
        bank_bics = {row[0] for row in BANKS}
        manifest_expected = _manifest_seedable_coverage("southern-africa")
        missing = [bic for bic in manifest_expected if bic not in bank_bics]
        assert not missing, (
            f"southern-africa SSI beneficiaries must also be seeded in BANKS so "
            f"Explore can show their settlement instructions: {missing}"
        )
...
class TestUnitedStatesSsiCoverage:
    ...
    def test_united_states_banks_have_seeded_ssi_records(self):
        manifest_expected = _manifest_seedable_coverage("united-states")
        generated_expected = {bic: (bank_name, currencies) for bic, bank_name, currencies in UNITED_STATES_SSI_COVERAGE}
        assert generated_expected == manifest_expected
        seeded = {}
        for record in SSI_RECORDS:
            seeded.setdefault(record[0], set()).add(record[2])
    ...
    def test_united_states_banks_are_in_the_bank_directory(self):
        ...
        missing = [bic for bic in manifest_expected if bic not in bank_bics]
        assert not missing, (
            f"united-states SSI beneficiaries must also be seeded in BANKS so "
            f"Explore can show their settlement instructions: {missing}"
        )
...
# ---- autopilot-generated coverage tests: luxembourg-wave1 ----
LUXEMBOURG_WAVE1_SSI_COVERAGE = [
    ("BCEELULLXXX", "Banque et Caisse d'Epargne de l'Etat, Luxembourg", {"AUD", "CAD", "CHF", "CNY", "CZK", "DKK", "GBP", "HKD", "HUF", "ISK", "JPY", "MXN", "NOK", "NZD", "PLN", "RON", "SEK", "SGD", "TRY", "USD", "ZAR"}),
]

class TestLuxembourgWave1SsiCoverage:

    def test_luxembourg_wave1_banks_have_seeded_ssi_records(self):
        manifest_expected = _manifest_seedable_coverage("luxembourg-wave1")
        generated_expected = {bic: (bank_name, currencies) for bic, bank_name, currencies in LUXEMBOURG_WAVE1_SSI_COVERAGE}
        assert generated_expected == manifest_expected
        seeded = {}
        for record in SSI_RECORDS:
            seeded.setdefault(record[0], set()).add(record[2])
    ...
    def test_luxembourg_wave1_banks_are_in_the_bank_directory(self):
        bank_bics = {row[0] for row in BANKS}
        manifest_expected = _manifest_seedable_coverage("luxembourg-wave1")
        missing = [bic for bic in manifest_expected if bic not in bank_bics]
        assert not missing, (
            f"luxembourg-wave1 SSI beneficiaries must also be seeded in BANKS so "
            f"Explore can show their settlement instructions: {missing}"
        )

    def test_luxembourg_wave1_seeded_records_are_semantically_valid(self):
        _assert_manifest_region_records("luxembourg-wave1", SSI_RECORDS, BANKS)
# ---- end autopilot-generated coverage tests: luxembourg-wave1 ----
