...
def test_tadhamon_bank_correspondent_list_matches_seed_rows():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-tibk-2026-09-14.json").read_text()
    )
    ...
    assert len(rows) == evidence["source_snapshot"]["route_count"] == 37
...
def test_tadhamon_rows_are_excluded_by_the_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "TIBKYESAXXX"
    ).all()
    assert len(rows) == 37
    assert all(not _is_routable_ssi(row) for row in rows)
    currencies = {row.currency for row in rows}
    assert all(
        suggest_from_ssi(db_session_clean, "TIBKYESAXXX", currency, "YE") == []
