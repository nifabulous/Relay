...
def test_al_sydbank_live_table_matches_bic_only_seed_rows():
    evidence = json.loads(
        (ROOT / "scripts/ssi-autopilot/evidence/ssi-wave65-albadkkk-2025-12-16.json").read_text()
    )
    ...
    assert len(rows) == evidence["source_snapshot"]["route_count"] == 24
...
def test_al_sydbank_rows_are_excluded_by_production_selector(db_session_clean):
    rows = db_session_clean.query(SSI).filter(
        SSI.beneficiary_bic == "ALBADKKKXXX"
    ).all()
    assert len(rows) == 24
    assert all(not _is_routable_ssi(row) for row in rows)
    currencies = {row.currency for row in rows}
    assert all(
        suggest_from_ssi(db_session_clean, "ALBADKKKXXX", currency, "DK") == []
