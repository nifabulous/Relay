...
def test_complete_input_mode_fails_closed_instead_of_reviewing_a_partial_diff(
    ...
    payload.write_text("untrusted diff" * 20, encoding="utf-8")

    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    monkeypatch.setattr(
        codex_responses,
        "request_response",
        lambda *args, **kwargs: pytest.fail("partial review reached the model"),
    )
    exit_code = codex_responses.main(
        [
            "--model", "gpt-5.3-codex",
