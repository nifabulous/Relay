...
require_text '.github/workflows/loopkeeper-pr-review.yml' \
  'commits/${RUN_HEAD_SHA}/pulls?per_page=100'
require_text '.github/workflows/loopkeeper-pr-review.yml' \
...
: >"$SELECTOR_OUTPUT"
: >"$SELECTOR_SUMMARY"
run_loopkeeper_selector '[]' "$SELECTOR_OUTPUT" "$SELECTOR_SUMMARY" || \
...
: >"$SELECTOR_OUTPUT"
: >"$SELECTOR_SUMMARY"
run_loopkeeper_selector '[{"number":1},{"number":103}]' "$SELECTOR_OUTPUT" "$SELECTOR_SUMMARY" || \
  fail 'Loopkeeper selector should fail closed without publishing a partial target set.'
...
check_matching_completed_ci_head_reaches_model() {
  ...
  # Build the syntactically valid address from pieces. The PR diff itself is
  # sanitized before reaching the reviewer, so keeping the address contiguous
  # in source would turn the raw fixture into the expected marker in review
  # evidence. At runtime the fixture still contains a real email-shaped value.
  raw_email_check_name="$(printf '%s@%s' 'frontend-check-owner' 'example.test')"
