...
def _expand_batch13_source_rows() -> list[tuple[str, ...]]:
    """Read the bounded batch-13 Tadhamon Bank ledger without executing seed.py."""
    path = REPO_ROOT / "app" / "services" / "seed_ssi_batch13_1.json"
    groups = json.loads(path.read_text(encoding="utf-8"))
    rows: list[tuple[str, ...]] = []
    real_note = _SOURCE_CONSTANTS.get("_SSI_REAL_NOTE", "")
    bic_only_note = (
    ...
    for node in tree.body:
        ...
        for element in node.value.elts:
            if (
                ...
                and element.value.func.id in {"_ssi_batch4_records", "_ssi_batch5_records", "_ssi_batch6_records", "_ssi_batch7_records", "_ssi_batch13_records"}
            ):
                rows.extend({
                    ...
                    "_ssi_batch13_records": _expand_batch13_source_rows,
    ...
    for node in tree.body:
        ...
        for i, e in enumerate(elts):
            if (
                ...
                and e.value.func.id in {"_ssi_batch4_records", "_ssi_batch5_records", "_ssi_batch6_records", "_ssi_batch7_records", "_ssi_batch13_records"}
            ):
                continue
